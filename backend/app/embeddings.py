...
PILLOW_WARN_PIXELS = Image.MAX_IMAGE_PIXELS  # ~89.5 Mpixel (Pillow default warn)
...
def get_model(settings):
    ...
    with _load_lock:
        if _state.get("key") != key:
            ...
            model.eval()
            _state.update(key=key, model=model, preprocess=preprocess, device=device)
